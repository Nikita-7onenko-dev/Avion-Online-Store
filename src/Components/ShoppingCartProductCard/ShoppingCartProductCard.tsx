...
export default function ShoppingCartProductCard({product, quantity, setQuantity}: Props): React.JSX.Element {
  ...
  return (
    <div className={styles.productCard}>
      <Link to={`/${product._id}`} className={`${styles.imageWrapper} ${product.aspectRatio === '4/5' ? styles.small : styles.wide}`}>
        <img 
          src={product.image}
          alt={product.name} 
          loading='lazy'
      ...
      <div className={styles.productDescription}>
        ...
        <QuantityInput quantity={quantity} setQuantity={setQuantity} productId={product._id} />
      </div>
    </div>
  )
}